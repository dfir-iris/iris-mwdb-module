...
import iris_mwdb_module.IrisMwdbConfig as interface_conf
from iris_mwdb_module.mwdb_handler.mwdb_handler import MwdbHandler


class IrisMwdbInterface(IrisModuleInterface):
    """
    Provide the interface between Iris and MWDBHandler
    """
    name = "IrisMwdbInterface"
    ...
    def pipeline_handler(self, pipeline_type, pipeline_data):
        """
        Receive data from the main pipeline and dispatch to MWDB handler
        :param pipeline_type:
        :param pipeline_data:
        :return:
        """
        ...
        try:
            ...
            if self._evidence_storage:
                ...
                if configuration.is_success():
                    ...
                    mwdb_handler = MwdbHandler(mod_config=self.module_dict_conf,
                                                       server_config=self.server_dict_conf,
                                                       evidence_storage=self._evidence_storage,
                                                       input_data=input_data,
                                                       logger=self.log)

                    ret = mwdb_handler.import_evidence()
                    if not ret:
